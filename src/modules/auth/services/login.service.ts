...
export class LoginService {
  ...
  async execute(data: LoginRequestDto): Promise<LoginResponseDto> {
    ...
    const accessExpiresIn  = this.configService.getOrThrow<string>('app.jwtExpiresIn');
    const refreshSecret    = this.configService.getOrThrow<string>('app.jwtRefreshSecret');
    const refreshExpiresIn = this.configService.getOrThrow<string>('app.jwtRefreshExpiresIn');
  ...
  private parseTtl(expiresIn: string): number {
    ...
    switch (unit) {
      ...
      default:  return parseInt(expiresIn);
    }
  }
}
